...
**Ledger 0001 — filed v0.17, ⚠ PARTIAL.** Four catalogued contradictions triaged: one fact-conflict (the birth year), one **modality-retype** (the transformation — the corpus held as `is` what was actually `saw`), one **malformed value** and one **undiagnosable for want of a declared query**. The last two are outside NAS-C13's three classes and are proposed as amendments to it. **Evidence grade D**: n=4, from a secondary record, because the corpus itself is no longer on disk — which is its own entry in the argument, a projection filed in place of the thing it projects. NAS-C13 stays *not yet falsified* rather than supported until the corpus is re-exported and the full set run (§16.2).
...
What the partial pass already returned, and would not have without being run: **two outcome classes the triage does not have** — *malformed value* (a contradiction inside one statement, caught structurally by TIME-1 rather than by triage) and *undiagnosable* (where no query is declared, the triage cannot run at all — which is GRAPH-4's justification stated from the other end). Both are proposed amendments to NAS-C13, both standing on n=4.

The corpus's own contradictions remain the acceptance tests: the model must make them impossible or loudly visible, and on the four available it does — one structurally impossible, two caught by GRAPH-3/MODAL-2, one caught by GRAPH-4's absence being the diagnosis.
### 16.3 Write a scene
