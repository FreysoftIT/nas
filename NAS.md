...
**Working draft — v0.22 (August 2026)**
...
**Changed in v0.22 — the interactive profile gains a fourth flip, and it is Principle I's.** Reading a real engine's fairness invariants surfaced an apparent contradiction with the founding principle: NAS says *nothing is canon until observed* (deferred commitment), and a fair interactive work says *the truth is written before you play it* — because an answer invented at the moment the player asks means the mystery was never solvable. Both hold, and neither yields; **they are talking about different units.** In a novel the collapse unit is the scene. In an interactive work it is the **unit of play** for the world and the **beat** for the reader: an engine commits the truth of one op or case at the moment it opens, because that is what the player will interrogate, while everything beyond it — untriggered dormant material, unpulled threads, unlanded consequence — **stays cloudy.** That is Principle I with a coarser grain on one axis and a finer one on the other, and §10's two-fold rule already carries it with no special case. Nothing in §14.2 changes. Also added: a **terminology hazard** at the head of the profile, because in this medium *canon* usually names the file of everything established **to the player** — which may contain binding falsehoods — and NAS's rules that say canon mean the **truth store**. Reading those two the wrong way round silently inverts the observer model the profile exists to protect. The profile's own version stamp had meanwhile read v0.15 while carrying v0.18 material; recorded in place rather than quietly corrected (ledger 0021's lesson, fifth instance of the class this week).

**Changed in v0.21 — a set difference needs two sides, found by an implementer.** GRAPH-11 amended, and the amendment came from **Alter-G building it** rather than from this corpus. Their hook is a deterministic bash set-difference against their own namespace declaration; across six active namespaces it produced **one** full check (`op`), one correct singleton (`world`), and **four namespaces with no dangling detection at all** — because the rule specified how to decide `requires_node` and said **nothing about how a namespace's *references* are recognised.** Here that question is invisible: every id is a distinctive `<ns>_` token that greps from anywhere, so the second side of the difference is free and the requirement never surfaced. Where ids are folder names, filenames and section headings, *"find every reference to thread X"* has no mechanical recipe at all. So a `true` namespace now declares **two** recipes — `defines` and `references` — and one with `defines` alone is **`uncheckable`**, reported separately from passing and never counted as covered. That third state is **REG-1's argument arriving one level down**: those four namespaces emit a green line that is neither dormant nor passing, since the check ran, examined something real, and cannot find the defect it exists to find — the *ran-against-a-declaration* category, occurring inside GRAPH-11's own implementation. And the substrate assumption travelled before the gap did: Alter-G's first declaration copied this project's token-regex recipe style and **matched zero real content**, which is their finding about this document — *structural anchor versus token pattern is a property of the substrate, not of the rule*, and nothing here said the two were separable (ledger 0021).

**Changed in v0.20 — the graph gets referents.** The dormancy pass left two repairs and both were larger underneath than on the surface. **OBS-1 was violated in writing**: `fact_burn_rate` — the most-referenced fact in the project, load-bearing in three scenes — carried `canonised_in: decree(2026-08-10, "load-bearing before any scene observes it")`. The breach was typed into the field designed to hold the observation record, and went unread for the life of the corpus because OBS-1 is tiered `structural`, and structural is the tier nobody checks. The observation had been in ch03's prose since v0.18 (*"the cadence people can't hold"*, *"she'd held that line for six years"*); only the `info_op` was missing — the exact mirror of ledger 0018, where the declaration existed and the prose did not. **GRAPH-11** (§7.5) closes the other half: every id namespace declares `requires_node`, settled by one test — *does any fold read state from its members* — so that dangling references and ids-in-place stop being indistinguishable, which was the real defect behind ledger 0019's 49-of-64. The declaration is machine-readable in the manifest because the check must stay a **set difference with no model in the loop** (Alter-G's condition for accepting it into M1). Its repairs: `fact_kes_year_seven`, which existed only as the target of a `tensions_with` edge — *a causal edge pointing at nothing* — and registries for the ten setups and thirteen facts that had no artifacts at all. The setup gap is the **cause** of the defect RENDER-1 was minted to catch. Both registries hold only the authored half and state the query for the derived half, because a registry that stored plant sites or canonisation sites would be GRAPH-2 arriving as the fix. And the fact registry made the sharpest distinction of the pass: **`reveal`/`collapse` canonise, `foreshadow`/`mislead` do not**, so three referenced facts are still **clouds** and declare a candidate space rather than content — writing a value for an uncollapsed fact is SCENE-3's authored snapshot, one layer up. **MODAL-4 amended**: it said *"carrying modality `must`"* without saying whose, and §7.8's two-place modality makes that two different rules; the corpus had been applying it to *read* modality for six versions, correctly and unspecified.
...
**Binding reconciles in both directions — PILLAR-2, added v0.18.** Preconditions radiate backward and PILLAR-1 checks them. **Postconditions radiate forward and nothing checked them at all**, which went unnoticed for as long as no pillar had ever bound: an unbound pillar's forward constraints are inert, so the gap is invisible until the moment it matters.

> **A bound pillar's postconditions are satisfied by the deltas its bound scene emits.** A postcondition with no emitting delta is *undelivered*, not implied.

This is CONTRACT-1's reconciliation, which chapters have had since v0.2, finally asked of the object that generates the most forward obligation in the system. *Found by binding the first pillar in either project:* three of six declared postconditions turned out never to have been emitted by the scene that was supposed to deliver them, and the scene had passed every other gate. **Optionality hides this — a pillar left `approaching` is never wrong about its future.**
...
**A projection can leak — where it is written matters as much as what it may do (added v0.18, register: GRAPH-10).** GRAPH-9 governs *direction*: reads never write. It says nothing about *placement*, and until now neither did anything else here:

> **A materialized projection inherits the union of the scopes it selected from, and may be written only where every reader of that location already holds at least that scope. A view spanning several observer scopes is never materialized inside one of them.**

The failure is quiet and total. Per-observer readings live in the **node's** `scopes` list (§7.8) — graph-side, correctly. But an observer record is *derived* (FACET-2), and a conforming implementation may compute and store one. Store the wrong one in the wrong file and every observer's reading of that fact — including canon's — is now sitting inside a file that one narrow-scoped reader can open. **Nothing in GRAPH-2, GRAPH-4 or GRAPH-9 forbids it.** The system that exists to keep a character from knowing what they cannot know would have handed it over in a generated view.

**An id is not a node — and which ids are (GRAPH-11, added v0.20).** The dormancy pass counted every `fact_*`, `val_*`, `setup_*`, `loc_*`, `rv_*` and `char_*` id referenced anywhere in a live corpus: **64 referenced, 15 defined, 49 with no defining artifact** (ledger 0019). Some of those are correct by design — reader values are declared inline and giving them nodes would assert a persistent reader-state object that §3.5 explicitly denies exists. Others were plain dangling references. **Nothing distinguished the two, and that indistinguishability was the defect** — not the count.

So each namespace declares `requires_node`, and the declaration is settled by one test rather than by taste:

> **A namespace requires a defining artifact if and only if some fold *reads* state from its members.** If the id is a label consumed only by the artifact that names it, and nothing downstream ever dereferences it, it is a string. A namespace does not earn a node because it feels important; it earns one when something downstream would otherwise have to guess.

Two consequences worth stating. **An undeclared namespace is an error, not a default** — silence is what produced the 49, and REG-1 already establishes that silence must never read as compliance. And `requires_node: true` means *must be defined*, not *must be a file*: valences are defined inline within their agent, because a valence has no existence apart from one and a separate file would need a hand-maintained owner edge, which is GRAPH-2 arriving through the back door.

This is the rule that turns referential integrity into a **set difference** — collect the ids, subtract the resolvable ones, report the remainder — which is why the declaration must be machine-readable and lives in the manifest rather than in this prose. Alter-G accepted it into M1 on exactly that condition: a deterministic script, no model in the loop.

Its first two repairs were both real. `fact_kes_year_seven` existed only as the target of a `tensions_with` edge — **a causal edge pointing at nothing**, in the file for the fact it is supposed to be in tension with. And ten setups had zero artifacts between them, which is the *cause* of the defect RENDER-1 was minted to catch: `setup_one_more` could be declared planted and never written, because a setup had no artifact to be absent from.

**A set difference needs two sides, and the rule only ever specified one — amended v0.21, from outside.** Alter-G implemented GRAPH-11 as a deterministic hook against their own declaration, and the implementation exposed the gap by refusing to paper over it. Their six active namespaces produced **one** full set difference (`op`), one degenerate-but-correct singleton check (`world`), and **four namespaces with no dangling detection at all** — `sitting`, `char`, `thread`, `node`.

The cause is not their declaration. It is this rule. GRAPH-11 says how to decide whether a namespace needs nodes, and says **nothing about how a namespace's *references* are recognised.** In this corpus that question is invisible, because every id here is a distinctive token: `fact_burn_rate` is greppable from anywhere, so collecting the referenced set is free and the requirement never surfaced. In a substrate where ids are folder names, filenames and section headings, *"find every reference to thread X"* has no mechanical recipe at all — a thread is referenced by prose that mentions it.

> **A `requires_node: true` namespace declares two extraction recipes: `defines` and `references`.** A namespace with `defines` and no `references` is **`uncheckable`** — it can report that its definitions are well-formed and nothing more. Uncheckable is reported separately from passing and never counted as covered.

The third state is the point, and it is REG-1's argument arriving one level down. Those four namespaces emit a green line that is **neither dormant nor passing**: the check ran, it examined something real, and it is structurally incapable of finding the defect it exists to find. That is exactly the *"ran against a declaration rather than the artifact"* category REG-1 was minted to separate out — now occurring **inside GRAPH-11's own implementation**, between two rules written six days apart.

**And the assumption travelled before the gap did.** Alter-G's first declaration used token regexes — `char_[a-z0-9_]+`, `thread_[a-z0-9_]+` — copied from the recipe style in this project's manifest, and their own review found the patterns matched **zero real content**. Their diagnosis is about this document, not theirs: **structural anchor versus token pattern is a property of the substrate, not of the rule.** A conforming implementation inherited this corpus's shape along with its rule, because nothing here said the two were separable.

Concretely: a character who believes a lie holds `read_modality: is` where canon holds `saw`. That gap is §3.2's irony and §7.8's deception. **Materialise the gap into that character's own file and the deception is in the liar's hands.**
...
*(Reference corpus — **corrected v0.17, and the correction is the interesting part.** This section cited ledger 0001's kicker as a demotion found in the wild: the written documents holding as *is* what was actually *saw*. The full audit found **zero modality-retypes in the corpus.** The kicker was real as *author testimony* — canon did live only in her head — but it never became a retype on paper, because the documents never asserted the wrong thing. What looked like a hardened attestation was a **query-divergence** (ledger 0001, Q1): one document answering *what was done and by whom*, another answering *did she consent*, both correct. The class this section was written to illustrate is, on the only evidence available, empty. Retained as the mechanism's clearest description; no longer claimed as an observed instance.)*
...
| GRAPH-10 | A materialized projection inherits the union of the scopes it selected from, and may be written only where every reader of that location already holds at least that scope. A view spanning several observer scopes is never materialized inside one of them | structural | invariant | — |
| GRAPH-11 | Every id namespace declares `requires_node`, by one test: does any fold **read** state from its members. A referenced id in a `true` namespace with no defining artifact is a **dangling reference**; in a `false` namespace it is a label and resolves to itself. Undeclared namespace = error, not default. **Amended v0.21:** a `true` namespace declares *two* extraction recipes — `defines` and `references` — because a set difference needs both sides. A namespace with a `defines` recipe and no `references` recipe is **`uncheckable`**, reported separately from passing, never counted as covered | gate | invariant | — |
...
| MODAL-4 | A statement at `institutions` layer or above **read as `must` by some observer** while canon holds it otherwise, or carrying canonical `must` with no world derivation, is surfaced at milestone review: a law people made, presenting to someone as a law of nature. **Amended v0.20 — the rule said "carrying modality `must`" without saying *whose*, and two-place modality (§7.8) makes that two different rules** | judgment | default | — |
...
**Ledger 0001 — filed v0.17, complete.** All 20 documents, 78,821 words, every contradiction triaged. **Nine items: 3 fact-conflicts, 2 query-divergences, 0 modality-retypes, 1 malformed value, 2 undiagnosable.** Two-thirds are not fact-conflicts, and the two most consequential — the story's moral centre and an authority ambiguity affecting every Modern Era reference — are both outside that class. **Evidence grade C** (n=9, primary source, classified by the claim's own author).

Three findings that changed the document. **The transformation "contradiction" is a false positive** — she volunteered, her father administered, the purpose was protection, and one paragraph in `summary_revision` states all three; the profile and the timeline were answering different questions. The catalogued fix would have deleted a true fact, which is NAS-C13's prediction holding harder than it was stated. **Zero modality-retypes exist in the corpus**, so §7.8's citation of this entry was corrected. And **every fact-conflict was introduced by the v1→v2 revision pass** — the v1 documents are internally and mutually consistent, and the v2 profile is coherent *on a wrong timeline*, which no intra-document read can catch.

Its contents, as the document has cited them since v0.4: the book-bible corpus audit — the 1763/1770 birth-year contradiction (`canonical_cause: NAS-C1`, confirms; GRAPH-2 would-have-caught), the forced-vs-voluntary transformation conflict, the misfiled canon docs (§11's evidence). The author's own account belongs in this entry as NAS-C9 evidence: the contradiction-hunting was done by hand, at the cost of burnout — and the v1→v2 revision cycle shows the predicted flattening drift (idealization of the protagonist, the agency-removing retcon, the late-added deuteragonist receiving structural significance but no interiority).
...
| 9 | Voice as object | ✅ **RATIFIED v0.14**, amended — then the *object* **retired in v0.17**. Voice attaches to facets, not characters: Wren speaks one way to the Practice and another to Tal, and as a character constant the most characteristic thing about dialogue reads as a lint violation. That amendment left the standalone `VoiceProfile` with nothing to do, so the interlock test found it the register's only stack. **`voice` survives as a facet field**; the object does not. A dedup, not a deletion |
...
**In two sizes.** The small one is done: v0.13's seed thread decomposes a story into the model on the page — nodes, edges, layers, modality, a faction, an agent, a pillar, a chapter contract, a scene beat, and one retcon cone walked to the bottom. It cost fourteen words of fiction and it found a real defect (the layer fork of §7.3, invisible until the schema demanded the field). The large one is **⚠ blocked, not merely pending.** Ledger 0001 was filed in v0.17 as **PARTIAL** — four catalogued contradictions triaged, evidence grade D — because **the corpus is no longer on disk.** The full pass needs it re-exported; until then NAS-C13 has been tested at n=4 from a secondary record, which is enough to keep it unfalsified and nowhere near enough to support it.

What the pass returned that argument could not: **two outcome classes the triage does not have** — *malformed value* (a contradiction inside one statement, caught structurally by TIME-1 rather than by triage) and *undiagnosable* (where no query is declared, the triage cannot run at all — which is GRAPH-4's justification stated from the other end). Both are amendments to NAS-C13, now standing on the full corpus. And a third finding no protocol anticipated: **zero modality-retypes**, in the class §7.8 was citing this very entry for.

✅ **The large one is done too, v0.17.** The corpus was found at `Desktop/Novel/BookBible` — all 20 documents, 78,821 words — and fully triaged in **ledger 0001**, open since v0.4.

The corpus's own contradictions were the acceptance tests: *the model must make them impossible or loudly visible.* **Eight of nine are addressed and four are impossible to represent** — the birth year (one node, age derived), the malformed era header (an interval ending before it starts), and both authority ambiguities (a view without a query is not a view; folders stop carrying precedence). The ninth is a wrong-but-well-formed date, caught by fold rather than by schema, which is the correct division of labour.

Three results worth more than the score. **The transformation "contradiction" was a false positive** — one paragraph in `summary_revision` states voluntary, by the father, for protection, all three at once; two documents were answering different questions. The correction the author had planned would have deleted a true fact, which is NAS-C13's prediction holding harder than it was written. **Zero modality-retypes exist in the corpus**, so §7.8's citation of this entry was corrected rather than defended. And **every fact-conflict was introduced by the v1→v2 revision pass**: the v1 documents are internally and mutually consistent, and the v2 profile is coherent *on a wrong timeline* — systematic drift, not sloppiness, which is exactly why it survived review.
### 16.3 Write a scene
...
*v0.22 — working draft. Iterate by editing this file. Nothing here is sacred except §0's problem statement, the §0.1 seed, and the §2 Triad — challenge everything else. The rest earns its place through the ledger, or leaves.*
